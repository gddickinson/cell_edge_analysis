...
class VisualizationPanel(QWidget):
    ...
    def plot_results(
        ...
        if curvature_data is not None and fluorescence_data is not None:
            ...
            self._plot_intensity_profile(fluorescence_data)
    ...
    def _plot_intensity_profile(
        ...
        """Plot intensity profile along membrane."""
        self.profile_fig.clear()
        ax = self.profile_fig.add_subplot(111)
        
        # Plot mean intensity vs position
        positions = np.arange(len(fluorescence_data.intensity_values))
        ax.plot(positions, fluorescence_data.intensity_values,
                'b-', linewidth=1)
        ...
        ax.set_xlabel('Position Along Membrane')
